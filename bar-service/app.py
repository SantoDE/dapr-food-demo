...
def handle_pour():
    """Pour beers — called via Dapr service invocation from the order workflow."""
    try:
        data = request.get_json()
        order_id = data["order_id"]
        customer_name = data["customer_name"]
        items = data.get("beers", [])

        print(f"🍺 Bar received order #{order_id} for {customer_name}: {items}", flush=True)
